...
export const uploadCover = (coverData) => async (dispatch, getState) => {
  try {
    ...
    dispatch(alertActions(error.response.data.message, "danger", 7000));
  }
};
...
export const updatePersonal = (userData) => async (dispatch, getState) => {
  try {
    ...
    dispatch(alertActions(response.data.message, "success"));

    dispatch(fetchCurrentUserDetail());
  } catch (error) {
    const { message } = error.response.data;
    if (typeof message === "string")
      return dispatch(alertActions(message, "danger"));
    message.map((message) => dispatch(alertActions(message, "danger")));
  }
...
export const changePassword = (passwordData) => async (dispatch) => {
  try {
    ...
    setTimeout(() => {
      ...
      dispatch(alertActions("Successfully logged out from App", "success"));
...
export const changeEmail = (emailData) => async (dispatch) => {
  try {
    ...
    dispatch(alertActions(error.response.data.message, "danger"));
  }
};
...
export const deleteUser = (deleteData) => async (dispatch) => {
  try {
    ...
    setTimeout(() => {
      ...
      // window.location.href = '/login';
      dispatch(alertActions("Sorry to see you go :(", "success"));
    }, 5000);
  } catch (error) {
    const { message } = error.response.data;
    if (typeof message === "string")
...
export const createPost = (post) => async (dispatch, getState) => {
  try {
    ...
    dispatch(setErrors(error.response.data.message));
    dispatch(alertActions(error.response.data.message, "danger", 6000));
  }
  const auth = getState().auth;
...
export const likePost = (id) => async (dispatch, getState) => {
  try {
    ...
    dispatch(alertActions(error.response.data.message, "danger", 3000));
  }
